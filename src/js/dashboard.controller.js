app.controller('DashboardController', function ($scope, $timeout, RequestApis, global, FileSaver, Blob) {
    ...
    global.currentUser((response) => {
        $scope.personel = response.personnelInfo;
        $scope.getSideBar($scope.sideBarItems[0]);
    })
    ...
    //============ side bar creation ======================
    $scope.sideBarItems = [
        {Id: 0, Title: 'اطلاعات من', selected: false},
    ...
    $scope.Completetraffic = function (item) {
        ...
        RequestApis.RC(`PersonRollCalls/GetPersonRollWithStateList?Person=${$scope.personel.Id}&Date=${$scope.convertToMiladi(item.startDayPersian)}`, 'Post', '', '', {
            "pageNumber": 1,
            "pageSize": 1000
        }, function (response) {
    ...
    $scope.saveReportPdf = function (searchItem) {
        ...
        RequestApis.RC('Reports/GetCompositeSearchReportStimule', 'Post', '', '', itemToSend, function (response) {
            ...
            FileSaver.saveAs(blob, "تفضیلی در تاریخ-" + new Date().toLocaleString() + '.pdf')
    ...
    $scope.downloadVacationCard = function (item) {
        RequestApis.RC("Reports/GetHourLeaveForPrint/", 'Get', '', 'arraybuffer', '', function (response) {
            if (response === 'Not Found') {
                ...
                const arr = new Uint8Array(response.data);
                const blob = new Blob([arr], {type: 'application/pdf'});
                FileSaver.saveAs(blob, "برگه مرخصی " + item.personName);
            }
        })
    }
    ...
    $scope.downloadMissionCard = function (item) {
        RequestApis.RC(`Reports/GetHourMissionForPrint/${item.id}`, 'Get', '', 'arraybuffer', '', function (response) {
            if (response === 'Not Found') {
                ...
                const arr = new Uint8Array(response.data);
                const blob = new Blob([arr], {type: 'application/pdf'});
                FileSaver.saveAs(blob, "برگه مأموریت " + item.personName + '.pdf')
            }
        })
    }
...
app.controller('abilityCtrl', function ($scope, $compile, $timeout, RequestApis, global, FileSaver, Blob) {
    ...
    $scope.removeItem = function (item) {
        RequestApis.HR(`miscellanea/personnel`, 'delete', '', '', {
            Id: item.Id,
