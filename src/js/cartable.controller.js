app.controller("cartableCtrl", function ($scope, $templateCache, $state, $stateParams, $http, RequestApis, $location, global) {
    //================== initial variable ========================
    $scope.inBox = false;
    $scope.requestAssessment = null;
    $scope.outBox = false;
    ...
    $scope.initFunc = function () {
        global.currentUser(function (data) {
            ...
            $scope.personel = data.personnelInfo;
    ...
    $scope.getCartablelist = function () {
        ...
        RequestApis.HR("cartables", 'Get', '', '', '', function (response) {
            if (response.status == 404) {
                global.Toast('warning', 'مورد برای نمایش وجود ندارد.')
                $scope.cartableList = {};
            } else {
                $scope.cartableList = response.data;
                RequestApis.HR("cartables/tags", 'Get', '', '', '', function (responsee) {
                    if (responsee.status == 404) {
                        $scope.tags = [];
                    } else {
                        $scope.tags = responsee.data
                    }
                })
            }
            $scope.inputMasks();
    ...
    $scope.setPage = function (item) {
        if ($scope.itemToSetCat != "tracking") {
            ...
            if (item.Request.AdditionalInfo != null) {
                $scope.urlParams = JSON.parse(item.Request.AdditionalInfo)
            } else {
                $scope.urlParams = JSON.parse(item.ParentRequest.AdditionalInfo)
            }
            $scope.urlParams.requestId = item.Request.Id;
            $scope.urlParams.stateBox = $scope.itemToSetCat;
            $scope.urlParams.behalfId = item.BehalfUserId;
